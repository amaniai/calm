...
centos_7_cloud = vm_disk_package(
    name="centos_7_cloud",
    description="Standard CentOS 7 cloud image",
    config={
        "name": "centos_7_cloud",
        "image": {
            "name": "centos_7_cloud",
...
class VMResources(AhvVmResources):
    ...
    cores_per_vCPU = 1
    disks = [
        AhvVmDisk.Disk.Scsi.cloneFromVMDiskPackage(centos_7_cloud, bootable=True)
    ]
    nics = [
...
class CentOS_7_Basic(Blueprint):
    """CentOS server 8.3 basic installation """

    services = [CentOS]
    packages = [PackageUbuntu, centos_7_cloud]
    substrates = [CentosVM]
    profiles = [Default]
